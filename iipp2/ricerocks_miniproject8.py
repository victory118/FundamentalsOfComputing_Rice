#codeskulptor link: http://www.codeskulptor.org/#user45_AMuqhWGflgkg18x_64.py
...
class Sprite:
    ...
    def collide(self, other_object):
        ...
        # return true if two objects are intersecting
...
def click(pos):
    global started, lives, my_ship, score
...
def draw(canvas):
    global time, started, score, lives, timer
    ...
    # check if ship collided with any rocks
    ...
    if lives == 0:
        ...
        soundtrack.pause()
        timer.stop()
        rock_group.difference_update(list(rock_group))
    else:
...
def rock_spawner():
    ...
    if len(rock_group) < 12:
        ...
        c_vel = 1.05**score
        rock_vel = [c_vel*(random.random() * .6 - .3), c_vel*(random.random() * .6 - .3)]
        rock_avel = random.random() * .2 - .1
        a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
        d = math.sqrt((my_ship.pos[0]-a_rock.pos[0])**2 + (my_ship.pos[1]-a_rock.pos[1])**2)
        if d >= a_rock.get_radius() + my_ship.get_radius() + 2*max(a_rock.get_radius(), my_ship.get_radius()):
            rock_group.add(a_rock)

# takes a set and canvas and calls the update and draw methods for each sprite in the group
def process_sprite_group(canvas, group):
    remove_set = set([])
    for s in group:
        s.draw(canvas)
        remove_sprite = s.update()
        if remove_sprite:
            remove_set.add(s)
...
def group_collide(group, other_object):
    ...
    for s in group:
        if s.collide(other_object):
            remove_set.add(s)
            an_explosion = Sprite(s.pos, [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
