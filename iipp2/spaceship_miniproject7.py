# codeskulptor link
#http://www.codeskulptor.org/#user45_rZr4dFh6jDsEAsg_79.py
...
class Ship:
    ...
    def update(self):
        ...
        if self.thrust:
            ...
            k_acc = 0.5
            self.vel[0] += k_acc*forward[0]
            self.vel[1] += k_acc*forward[1]
    ...
    def shoot(self):
        ...
        k_vel = 8.0
        missile_vel = [self.vel[0] + forward[0]*k_vel, self.vel[1] + forward[1]*k_vel]
...
def draw(canvas):
    ...
    canvas.draw_text("Lives", [50, 50], 28, "White", "sans-serif")
    canvas.draw_text(str(lives), [50, 85], 28, "White", "sans-serif")
    canvas.draw_text("Score", [675, 50], 28, "White", "sans-serif")
    canvas.draw_text(str(score), [675, 85], 28, "White", "sans-serif")
...
def rock_spawner():
    global a_rock
    max_vel = 7
    x_vel = 2*max_vel*(random.random() - 0.5)
    y_vel = 2*max_vel*(random.random() - 0.5)
    x_pos = random.random()*WIDTH
    y_pos = random.random()*HEIGHT
    max_angle_vel = 0.2
    angle_vel = 2*max_angle_vel*(random.random() - 0.5)
    a_rock = Sprite([x_pos, y_pos], [x_vel, y_vel], 0, angle_vel, asteroid_image, asteroid_info)
